...
OCR_IMAGE_SUFFIXES = {".png", ".jpg", ".jpeg", ".tif", ".tiff", ".bmp", ".webp"}
...
def ocr_to_documents(
    ...
    Args:
        pdf_input: Path to a PDF file or a folder containing PDF files.
        language: Tesseract language code, for example "eng".
    ...
    if source_path.is_dir():
        ...
        raise ValueError(f"Expected a PDF/image file or folder containing OCR-supported files: {input_path}")
    ...
    for source_file_path in file_paths:
        ...
        for page_number, image in enumerate(page_images, start=1):
            if page_number == 1 or page_number % 10 == 0 or page_number == total_pages:
                print(f"OCR progress: page {page_number}/{total_pages} for {source_file_path.name}")
        ...
        if loaded_pages == 0:
            ...
            print(f"OCR extracted text from:\n {source_file_path}\n With text from {loaded_pages} pages.")
...
def create_vectorstore(chunks: List[Document], embedding_model: HuggingFaceEmbeddings) -> FAISS:
    ...
    print(f"Building FAISS index from {len(chunks)} chunk(s). This can take time on CPU.")
